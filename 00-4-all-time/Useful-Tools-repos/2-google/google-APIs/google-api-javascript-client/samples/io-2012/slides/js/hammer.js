...
function Hammer(element, options, undefined) {
  ...
  (function () {
    ...
    for (var i = 0; i < vendors.length; i++) {
      for (var p in css_props) {
        ...
        if (vendors[i]) {
          prop =
            vendors[i] + prop.substring(0, 1).toUpperCase() + prop.substring(1);
        }
  ...
  var gestures = {
    ...
    drag: function (event) {
      ...
      _distance = Math.sqrt(
        _distance_x * _distance_x + _distance_y * _distance_y
      );

      // drag
      // minimal movement required
      if (
        (options.drag && _distance > options.drag_min_distance) ||
        _gesture == "drag"
      ) {
        // calculate the angle
        ...
        if (
          ((is_vertical && !options.drag_vertical) ||
  ...
  function handleEvents(event) {
    switch (event.type) {
      ...
      case "touchstart":
        ...
        var scrollTop =
          window.pageYOffset || element.scrollTop || document.body.scrollTop;
        var scrollLeft =
          window.pageXOffset || element.scrollLeft || document.body.scrollLeft;
