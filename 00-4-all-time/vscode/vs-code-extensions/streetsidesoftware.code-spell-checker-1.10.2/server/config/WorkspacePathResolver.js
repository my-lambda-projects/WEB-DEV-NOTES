...
const cspellConfig_1 = require("../config/cspellConfig");
...
function resolveSettings(settings, resolver) {
    ...
    function setOptions(defs) {
        const values = defs
            .filter((d) => !!d)
            .map((d) => d)
            .map((def) => ({ type: cspellConfig_1.defaultDictionaryType, ...def }));
...
function createWorkspaceNameToGlobResolver(folder, folders) {
    const folderPairs = [['${workspaceFolder}', folder.path]].concat(folders.map((folder) => [`\${workspaceFolder:${folder.name}}`, folder.path]));
